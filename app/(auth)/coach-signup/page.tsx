...
import { signUpAsCoach, signInWithGoogle, signInWithApple } from "@/lib/auth/actions";
...
export default function CoachSignupPage() {
  ...
  async function handleSignUp(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const result = await signUpAsCoach(email, password, fullName);
    if (result?.error) {
    ...
    <div className="min-h-screen bg-[#D7D7D7] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="border-2 border-black bg-[#D7D7D7] p-8">
          ...
          <div className="inline-block border border-[#007B6F] text-[#007B6F] text-xs px-2 py-0.5 mb-6 tracking-wider uppercase">
            Coach
          </div>

          <h1 className="text-xl font-semibold mb-1">Join as a coach</h1>
          <p className="text-sm text-black/60 mb-6">
            Build your profile, set your offers, and start coaching remotely.
          </p>
          ...
          <div className="space-y-3 mb-8">
            <form action={signInWithGoogle.bind(null, "coach")}>
              <button
                type="submit"
            ...
            <form action={signInWithApple.bind(null, "coach")}>
              <button
                type="submit"
